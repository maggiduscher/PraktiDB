...
--
-- Host: 127.0.0.1
-- Erstellungszeit: 07. Apr 2018 um 15:57
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddAngenommene`(IN `UserID` INT, IN `AngebotsID` INT)
    NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddUnternehmen`(IN `Name` VARCHAR(50), IN `Adresse` VARCHAR(50), IN `PLZ` VARCHAR(50), IN `Branche` VARCHAR(50), IN `Email` VARCHAR(50))
BEGIN
...
DROP PROCEDURE IF EXISTS `GetAllAngenommen`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllAngenommen`()
    NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetBewertung`(IN `UserID` BIGINT, IN `UnternehmensID` BIGINT)
    NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetBewertungenUnternehmen`(IN `UnternehmensID` BIGINT)
    NO SQL
SELECT * FROM tbuser_bewertung JOIN tbuser USING (biUserID) JOIN tbunternehmen USING (biUnternehmensID)
WHERE biUnternehmensID = UnternehmensID$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateUnternehmen`(IN `ID` BIGINT, IN `Texxt` TEXT, IN `Adresse` VARCHAR(50), IN `Branche` VARCHAR(50), IN `EMail` VARCHAR(50), IN `PLZ` VARCHAR(50), IN `Name` VARCHAR(50))
    NO SQL
BEGIN
...
(8, 'NMartin', 'admin', 'niklasm.cyber@gmail.com', 'Niklas', 'Martin', 'Am Heidekrug 33', '40724', 'ITA52', '1999-03-08', 'addb0f5e7826c857d7376d1bd9bc33c0c544790a2eac96144a8af22b1298c940', '');
...
ALTER TABLE `tbuser_bewerbungen`
 ADD KEY `biAngebotesID` (`biAngebotsID`), ADD KEY `biUserID` (`biUserID`);

--
-- Indizes für die Tabelle `tbuser_bewertung`
--
ALTER TABLE `tbuser_bewertung`
 ADD KEY `biAngebotesID` (`biUnternehmensID`), ADD KEY `biUserID` (`biUserID`);
...
--
ALTER TABLE `tbangebote`
MODIFY `biAngebotsID` bigint(20) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=5;
--
...
--
ALTER TABLE `tbuser`
MODIFY `biUserID` bigint(20) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=10;
--
